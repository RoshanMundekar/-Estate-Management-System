...
def landproerty1():
  ...
  con = dbConnection()
  cursor = con.cursor()
  cursor.execute("SELECT * FROM request1 WHERE user = %s AND status = %s AND booking = %s;",(user,status,booking))
  result1 = cursor.fetchall()
...
def book():
    ...
    con = dbConnection()
    cursor = con.cursor()
    cursor.execute("SELECT * FROM addland where user=%s and filenamepath=%s and Landprice=%s",(Uname,Imgname,lprice))
...
def booked():
    ...
    if request.method == "POST":
        ...
        val1 = (ccg, str(user) ,str(email),str(address),str(city),str(state))
        cursor.execute(sql1,val1)
        con.commit()
        ...
        ccg1="BOOK"
        print(ccg)
        sql2 = "UPDATE request1 SET booking = %s WHERE user = %s  AND email = %s AND maplocation = %s;"
        val2 = (ccg1, str(user) ,str(email),str(address))
        cursor.execute(sql2,val2)
        con.commit()
        return redirect(url_for("index"))
